...
func (p PurchaseOrder) Validate() error {
	...
	if err != nil {
		return fmt.Errorf("%s %w", "josn.Unmarshall()", err)
	}
	...
	for _, v := range ptps {
		...
		if v.Amount < 1 {
			return errors.New("the cmount of product can't be less than 1")
		}
		if v.UnitPrice < 0 {
			return errors.New("the product price can't be negative")
		}
	}
